...
const EditListing = () => {
    ...
    const fetchListing = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setContactPhone(data.ownerPhone || data.contactPhone || user?.phone || "");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (type === "products") ProductListingSchema.parse({ ...baseData, condition });
      else if (type === "services") ServiceListingSchema.parse(baseData);
      else RequestListingSchema.parse({ ...baseData, budget: Number(price) || 0 });
    } catch (zodError: any) {
    ...
    try {
       ...
       if (isAdmin) {
         ...
         updateData.ownerIsVerified = ownerIsVerified;
         updateData.lastEditedByAdmin = user?.email;
          ...
          {isAdmin && (
            <div className="pt-6 mt-6 border-t border-black/10 space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-sm uppercase tracking-widest text-textPrimary">Admin Moderation</h3>
              </div>

              <div className="grid gap-4 bg-muted/20 p-6 rounded-2xl border border-black/5">
                <div className="grid gap-2">
                  <Label htmlFor="status" className="text-xs font-bold uppercase tracking-wider text-textSecondary">Listing Status</Label>
                  <Select
                    value={status}
                    onValueChange={(val) => setStatus(val)}
                  >
                    <SelectTrigger className="rounded-xl border-black/5 bg-card h-12">
                      <SelectValue placeholder="Select Status" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="approved">Approved</SelectItem>
                      <SelectItem value="pending">Pending</SelectItem>
                      <SelectItem value="removed">Removed / Rejected</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex flex-wrap gap-8 pt-4">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <Label htmlFor="featured" className="text-xs font-bold uppercase cursor-pointer">Featured Listing</Label>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Switch
                      id="verified"
                      checked={ownerIsVerified}
                      onCheckedChange={(val) => setOwnerIsVerified(val)}
                    />
                    <Label htmlFor="verified" className="text-xs font-bold uppercase cursor-pointer">Verified Owner</Label>
                  </div>
                </div>
              </div>
            </div>
