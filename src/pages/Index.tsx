...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] pb-24 md:pb-0">
      ...
      {/* Hero area */}
      <section className="relative overflow-hidden pt-12 md:pt-20 pb-20 border-b border-black/5 bg-white">
        <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-[500px] h-[500px] bg-[#1F6FEB]/5 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-[#8A2BE2]/5 rounded-full blur-3xl" />
        ...
        <div className="container relative z-10 flex flex-col items-center text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-[#222222] text-balance leading-[0.95] animate-fade-up stagger-1">
            Trade with <span className="text-gradient">Style.</span> <br />
            Trade in <span className="text-[#00C48C]">Kashere.</span>
          </h1>
          
          <p className="mt-6 text-lg md:text-xl text-[#555555] max-w-xl font-medium text-pretty animate-fade-up stagger-2">
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-4">
          {categories.map((cat, i) => (
            <Link
              ...
              className="group flex flex-col items-center gap-3 rounded-3xl glass-card py-6 px-4 text-center transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 border border-white/50 animate-fade-up"
              style={{ animationDelay: `${i * 40}ms` }}
            >
              <div className="h-14 w-14 rounded-2xl bg-[#F7F8FA] flex items-center justify-center text-3xl shadow-inner group-hover:bg-primary/10 group-hover:scale-110 transition-all duration-300">
                {cat.icon}
              </div>
              <span className="text-[10px] font-black text-[#222222]/70 uppercase tracking-widest group-hover:text-primary transition-colors">{cat.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {recentlyViewed.length > 0 && (
        <section className="container pt-16 animate-fade-up stagger-1">
          <div className="flex items-center gap-3 mb-8">
            <div className="p-2.5 rounded-2xl bg-[#1F6FEB]/10 text-[#1F6FEB] border border-[#1F6FEB]/10 shadow-sm">
              <Clock className="h-6 w-6" />
            </div>
            <h2 className="text-2xl font-black text-[#222222] tracking-tighter">Your recent history</h2>
          </div>
      ...
      <section className="container pt-16 animate-fade-up stagger-2">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-yellow-400/20 text-yellow-700 border border-yellow-400/10 shadow-sm">
               <Sparkles className="h-6 w-6" />
            </div>
            <h2 className="text-2xl font-black text-[#222222] tracking-tighter">Premium Selection</h2>
          </div>
      ...
      <section className="container pt-16 pb-4 animate-fade-up stagger-3">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-black text-[#222222] tracking-tighter">New Arrivals</h2>
          </div>
          <Link to="/marketplace" className="text-xs font-black uppercase tracking-widest text-[#555555] flex items-center gap-2 hover:text-primary transition-colors">
      ...
      <section className="container pt-16 pb-8 animate-fade-up">
        <div className="flex items-center justify-between mb-8 border-b border-black/5 pb-4">
          <h2 className="text-2xl font-black text-[#222222] tracking-tighter">Campus Experts</h2>
          ...
          <div className="rounded-[2.5rem] bg-gradient-to-br from-white via-[#F7F8FA] to-white p-8 md:p-12 border border-white/80 shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4">
              <div className="flex flex-col gap-1">
                <h2 className="text-3xl font-black text-[#222222] tracking-tight leading-none italic">Community Pulse</h2>
                <p className="text-sm font-bold text-muted-foreground/60">What students are hunting for</p>
              </div>
              <Link to="/requests" className="btn-premium px-6 py-3 rounded-2xl text-white font-black text-xs uppercase tracking-widest shadow-xl">
                View All Requests
              </Link>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6">
              {requestsPreview.map((req, i) => (
                <Link key={req.id} to="/requests" className="group rounded-3xl bg-white p-6 border border-black/5 hover:border-primary/20 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 flex flex-col h-full" style={{ animationDelay: `${i * 100}ms` }}>
                   <div className="flex items-center justify-between mb-4">
                      ...
                      <p className="text-base font-black text-secondary tabular-nums tracking-tight">{formatPrice(req.budget)}</p>
                   </div>
                   <h3 className="font-black text-[#222222] text-sm leading-tight mb-3 group-hover:text-primary transition-colors line-clamp-2">{req.title}</h3>
                   <p className="text-xs text-[#555555] line-clamp-2 leading-relaxed font-medium mb-4">{req.description}</p>
