...
const CompleteSignup = () => {
  ...
  const { user, updateProfile, checkBusinessNameUniqueness, isAuthenticated } = useAuth();
  ...
  useEffect(() => {
    if (user?.businessName && user.businessName !== "" && user.businessName !== "Unknown User") {
        ...
        console.log(`[AuthRedirect CompleteSignup Effect] User: ${user?.email}, Role: ${role}, Redirecting to: ${redirectPath}`);
  ...
  const handleBusinessNameChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const unique = await checkBusinessNameUniqueness(val);
      setIsUnique(unique);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const role = user?.role || "user";
      const redirectPath = role === "admin" ? "/admin" : "/";
      console.log(`[AuthRedirect CompleteSignup Submit] User: ${user?.email}, Role: ${role}, Redirecting to: ${redirectPath}`);
      navigate(redirectPath, { replace: true });
    } catch (error: any) {
      console.error(error);
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          <Link to="/" className="inline-flex items-center gap-2 font-black text-3xl tracking-tight mb-4 italic group">
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground font-black shadow-xl group-hover:rotate-12 transition-transform duration-500">S</span>
            Siyayya
          </Link>
