  ...
  const captureCard = async (): Promise<string | null> => {
    ...
    try {
      ...
      const dataUrl = await toPng(cardRef.current, {
        quality: 1.0,
        pixelRatio: 2, // High res 1080p roughly depending on width
        canvasWidth: cardRef.current.clientWidth * 2,
        canvasHeight: cardRef.current.clientHeight * 2,
      });
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className="no-share absolute top-3 left-3 z-20 h-9 w-9 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-md border border-white/20 transition-all hover:bg-black/60 hover:scale-110 active:scale-95 group/sharebtn"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
          }}
      ...
      <DropdownMenuContent align="start" className="w-48 bg-[#161b2e]/95 backdrop-blur-xl border-white/10 text-white rounded-xl shadow-2xl p-1 z-[100]" onClick={(e) => e.stopPropagation()}>
         <DropdownMenuItem onClick={handleShareNative} className="gap-2 cursor-pointer focus:bg-white/10 hover:bg-white/10 rounded-lg">
           <Share2 className="h-4 w-4 text-blue-400" /> Share Image
         </DropdownMenuItem>
         <DropdownMenuItem onClick={handleWhatsAppShare} className="gap-2 cursor-pointer focus:bg-white/10 hover:bg-white/10 rounded-lg">
           <MessageCircle className="h-4 w-4 text-emerald-400" /> Share to WhatsApp
         </DropdownMenuItem>
         <DropdownMenuSeparator className="bg-white/10 opacity-50" />
         <DropdownMenuItem onClick={handleDownload} className="gap-2 cursor-pointer focus:bg-white/10 hover:bg-white/10 rounded-lg">
           <Download className="h-4 w-4 text-orange-400" /> Download PNG
         </DropdownMenuItem>
         <DropdownMenuItem onClick={handleCopyLink} className="gap-2 cursor-pointer focus:bg-white/10 hover:bg-white/10 rounded-lg">
