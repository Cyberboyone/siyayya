    ...
    <div className="min-h-screen bg-[#F7F8FA] pb-24 md:pb-0">
      ...
      <div className="container py-8 max-w-6xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black text-[#222222] tracking-tighter capitalize">{user?.businessName || user?.name}'s Dashboard</h1>
            <p className="text-sm font-bold text-muted-foreground/60 uppercase tracking-widest mt-1">Manage your campus trade profile</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-3 space-y-1 block overflow-x-auto md:overflow-visible whitespace-nowrap md:whitespace-normal pb-2 md:pb-0 scrollbar-none border-b md:border-b-0 md:border-r border-border pr-0 md:pr-4">
            <div className="flex md:flex-col gap-2 min-w-max md:min-w-0 px-1 md:px-0">
              {tabs.map((t) => (
                ...
                >
                  <t.icon className={`h-5 w-5 ${tab === t.id ? "text-primary-foreground" : "text-muted-foreground"}`} />
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <div className="md:col-span-9 min-h-[500px]">
            {isLoading ? (
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                <div className="rounded-2xl border border-primary/10 bg-gradient-to-br from-card to-secondary/30 p-6 flex flex-col sm:flex-row items-center sm:items-start gap-6 shadow-sm relative overflow-hidden">
                  ...
                  <div className="text-center sm:text-left z-10">
                    <div className="flex items-center justify-center sm:justify-start gap-1">
                      <h2 className="text-2xl font-bold text-foreground">{user?.businessName || user.name}</h2>
                      {user.isVerified && <VerifiedBadge />}
                    </div>
                    <p className="text-sm font-medium text-muted-foreground mt-0.5">{user.department}</p>
                    <div className="flex items-center justify-center sm:justify-start gap-2 mt-2 bg-background/50 w-fit sm:mx-0 mx-auto px-3 py-1 rounded-full border border-border/50">
              ...
              <div>
                ...
                {myProducts.length > 0 ? (
                  <div className="space-y-3">
                    {myProducts.map((p) => (
                      <div key={p.id} className={`rounded-xl border bg-card p-3 flex gap-3 ${p.isSold ? "opacity-50" : ""}`}>
                        <img src={p.image} alt={p.title} className="h-16 w-16 rounded-lg object-cover shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <h3 className="text-sm font-medium text-foreground truncate">{p.title}</h3>
                              <p className="text-sm font-semibold text-primary tabular-nums">{formatPrice(p.price)}</p>
                            </div>
                            {p.isSold && (
                              <span className="text-[10px] font-semibold bg-muted text-muted-foreground px-2 py-0.5 rounded-full shrink-0">SOLD</span>
                            )}
                          </div>
                      ...
                      <div key={s.id} className="rounded-xl border bg-card p-4">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-sm text-primary font-semibold tabular-nums mt-0.5">{s.priceLabel} {formatPrice(s.price)}</p>
                          </div>
                ...
                <div key={r.id} className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-primary font-bold tabular-nums mt-1">Budget: {formatPrice(r.budget)}</p>
                    </div>
                    <div className="flex gap-2">
                      <Link to={`/dashboard/edit/requests/${r.id}`}>
                        <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 shadow-sm">
                          <Edit className="h-3.5 w-3.5" /> Edit
                        </Button>
                      </Link>
                      <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 text-destructive hover:bg-destructive shadow-sm hover:text-destructive-foreground transition-colors">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2">
              ...
              {savedProducts.length > 0 ? (
                ...
                <div className="text-center py-16 text-muted-foreground border-2 border-dashed rounded-xl bg-card/50">
                  ...
                  <p className="font-semibold text-base text-foreground">No saved items yet</p>
                  <p className="text-sm mt-1 max-w-xs mx-auto">Tap the heart icon on any product to save it to your wishlist.</p>
                </div>
              )}
            </div>
          )}
                    ...
                    <div key={r.id} className="rounded-xl border bg-card p-5 shadow-sm">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-sm text-foreground">{r.ownerName}</p>
                            <p className="text-xs text-muted-foreground">Verified User</p>
                          </div>
                        </div>
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={s} className={`h-3.5 w-3.5 ${s <= r.rating ? "fill-yellow-500 text-yellow-500" : "text-muted-foreground/20"}`} />
                          ))}
                        </div>
                      </div>
          ...
          {tab === "settings" && (
            <div className="rounded-xl border bg-card p-6 md:p-8 space-y-6 shadow-sm animate-in fade-in slide-in-from-bottom-2 max-w-2xl">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-bold text-foreground">Profile Settings</h2>
                  <p className="text-sm text-muted-foreground mt-1">Update your profile information visible to others.</p>
                </div>
                {user && (
              ...
              <div className="grid gap-5">
                ...
                <div className="border-b border-border/50 pb-1">
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">Basic Information</h3>
                  <div className="grid gap-4">
                    <div className="space-y-1.5 mb-2">
                       <label className="text-sm font-semibold text-foreground ml-1">Profile Photo</label>
                    ...
                    {[
                      ...
                      <div key={field.key} className="space-y-1.5">
                        <label className="text-sm font-semibold text-foreground ml-1">{field.label}</label>
                        {field.key === "bio" ? (
                          <textarea
                            rows={3}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-foreground ml-1">Email Address <span className="text-xs text-muted-foreground font-normal">(read-only)</span></label>
                      <Input type="email" value={profileData.email} disabled className="bg-secondary/50 opacity-70 cursor-not-allowed" />
                    </div>
                  </div>
                </div>
                ...
                {/* Campus Info */}
                <div className="border-b border-border/50 pb-1">
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">Campus Information</h3>
                  <div className="grid gap-4">
                    {[
                ...
                <div className="pt-1">
                  <Button
                    className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-md w-full sm:w-auto h-11 px-8 rounded-xl font-semibold transition-all active:scale-[0.98]"
                    onClick={async () => {
                      setIsUpdatingProfile(true);
                      try {
                        await updateProfile({
