...
export const ProductCard: React.FC<ProductCardProps> = React.memo(({ product, index = 0 }) => {
  ...
  const createChatAndNavigate = React.useCallback(async () => {
    if (!authUser) return;
  ...
  }, [authUser, product, navigate]);

  const handleChatSeller = React.useCallback(async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    createChatAndNavigate();
  }, [isAuthenticated, authUser, createChatAndNavigate]);

  const handleBuyNow = React.useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    addToCart(product);
