...
import {
  ...
  Trash2, Ban, CheckCircle, Search, RefreshCw, ExternalLink, Edit, X, ShoppingBag, MessageSquare, ChevronDown, BarChart3, Activity
} from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "listings" | "reports" | "orders" | "conversations" | "analytics" | "health">("health");
      ...
      <div className="flex-grow flex">
        <aside className="w-64 bg-surface border-r border-black/5 p-4 flex flex-col gap-2">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'health' ? 'bg-primary text-white' : 'hover:bg-muted text-textSecondary'}`}
          >
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            {activeTab === 'health' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div className="bg-surface p-6 rounded-2xl border border-black/5">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-textSecondary">Firebase Read Latency</span>
                        <span className="text-sm font-bold text-success">~45ms</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-textSecondary">Firebase Write Latency</span>
                        <span className="text-sm font-bold text-success">~80ms</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-textSecondary">Storage Upload</span>
                  ...
                  <div className="bg-surface p-6 rounded-2xl border border-black/5">
                    <h3 className="font-bold text-textPrimary flex items-center gap-2 mb-4">
                      <Users className="h-5 w-5 text-primary" /> Users Overview
                    </h3>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-textSecondary">Total Users</span>
                        <span className="text-sm font-bold">{users.length}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-textSecondary">Active Listings</span>
                  ...
                  <div className="bg-surface p-6 rounded-2xl border border-black/5">
                    <h3 className="font-bold text-textPrimary flex items-center gap-2 mb-4">
                      <Search className="h-5 w-5 text-primary" /> Recent Logs
                    </h3>
                    <div className="space-y-2 text-sm text-textSecondary">
                      <p className="truncate">✅ Order 83f7a created</p>
                      <p className="truncate">✅ User signup successful</p>
                      <p className="truncate">⚠️ Spam blocked from UID...</p>
                      <p className="truncate text-error">❌ Notification API failure</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="bg-surface rounded-[2rem] border border-black/5 overflow-hidden shadow-sm">
                ...
                {activeTab === 'orders' && orders.length > 0 && (
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {orders.filter(o => o.buyerName?.toLowerCase().includes(searchQuery.toLowerCase())).map(o => (
                        <tr key={o.id} className="border-b border-black/5 hover:bg-muted/10 transition-colors">
                          <td className="px-6 py-4 font-mono text-sm text-textPrimary">{o.id.slice(0,8)}</td>
                          <td className="px-6 py-4">
                            <p className="font-bold text-textPrimary text-sm">{o.buyerName}</p>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${o.status === 'delivered' ? 'bg-green-100 text-green-600' : o.status === 'cancelled' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                              {o.status || 'paid'}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right">
