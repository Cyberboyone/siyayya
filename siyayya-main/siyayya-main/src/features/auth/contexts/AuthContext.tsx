...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginWithGoogle = React.useCallback(async () => {
    try {
      ...
      setSentryUser({ id: result.user.uid, email: result.user.email || undefined, name: result.user.displayName || undefined });
  ...
  const loginWithPhoneLite = async (name: string, phone: string) => {
    try {
      ...
      setSentryUser({ id: result.user.uid, name: name });
      trackSignUp('phone');
      return result.user;
    } catch (error: any) {
  ...
  const addPhoneToGoogleAccount = React.useCallback(async (phone: string) => {
  ...
  const value = React.useMemo(() => ({
    ...
    deleteAccount 
  }), [user, isAdmin, isLoading, logout, updateProfile, loginWithGoogle, loginWithPhoneLite, addPhoneToGoogleAccount, deleteAccount]);
